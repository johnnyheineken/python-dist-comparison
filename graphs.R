library(readr)
conda <- as.data.frame(t(read_csv("Documents/aLook/blog/python-comparison/conda.csv", 
                  col_names = FALSE, col_types = cols(X1 = col_skip()))), stringsAsFactors = FALSE)

clean <- as.data.frame(t(read_csv("Documents/aLook/blog/python-comparison/clean.csv", 
                  col_names = FALSE, col_types = cols(X1 = col_skip()))), stringsAsFactors = FALSE)

intel <- as.data.frame(t(read_csv("Documents/aLook/blog/python-comparison/intel.csv", 
                  col_names = FALSE, col_types = cols(X1 = col_skip()))), stringsAsFactors = FALSE)
...
p <- ggplot(create_graph_data('intel', '1000000'), aes(x=test, y=as.numeric(value))) +
  ...
  ylab('Best time (μsec)') + xlab('Speed test with array length of 1 000 000') +theme_minimal() + scale_fill_brewer(palette='Paired')
p
...
p <- ggplot(data_graph, aes(x=test, y=as.numeric(value))) +
  geom_bar(aes(fill=variable), stat="identity", position = 'dodge') +
  ylab('Best time (μsec)') + xlab('Speed test with array length of 100 000') +theme_minimal() + scale_fill_brewer(palette='Paired')
p
